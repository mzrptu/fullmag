...
|----|--------|------|-----------|------------|----------------------|
| **EIG-040** | Flat DSL | Dodać `study.eigenmodes()` do `world.py` | **P0 — krytyczny** | — | Użytkownik może w flat-script po `study.relax()` wywołać `study.eigenmodes(count=10, ...)` |
...
| **EIG-045** | Frontend Tree | Dodać node "Results → Eigenmodes" w `ModelTree.tsx` | **P1** | — | Po eigen run, w drzewie pojawia się gałąź z modami |
...
| **EIG-050** | GPU native | Stub ABI `fullmag_fem_eigen_solve()` w `native/include/fullmag_fem_eigen.h` | **P2** | — | ABI jest zdefiniowane, zwraca `NOT_IMPLEMENTED` |
...
study.run(1e-9)  # optional time-domain after
```

> **Uwaga:** Powyższy przykład pokazuje docelową składnię. Mieszany pipeline `relax → eigen → time-run` wymaga implementacji multi-stage queue w runtime — nie jest jeszcze wspierany. Sprint 1 skupia się na sekwencji `relax → eigenmodes` (bez kolejnego `study.run()` po analyze).

**Implementacja krok po kroku:**

#### Krok 1.1 — `world.py`: metoda `Study.eigenmodes()`

Plik: `packages/fullmag-py/src/fullmag/world.py`

Dodać metodę `eigenmodes()` na wzór istniejącej `relax()`:
...
| `apps/web/components/runs/control-room/ViewportPanels.tsx` | **ROZSZERZYĆ** — routing z tree selection do AnalyzeViewport | ~20 linii |
...
### Sprint 1 (teraz): Flat DSL + nanoflower script
1. `study.eigenmodes()` w `world.py`
2. Rozszerzenie `save()` o eigen outputs
3. Usunięcie blokad w `script_builder.py`
4. Serializacja eigen study → IR
5. Test: nanoflower_fem.py z eigenmodes
6. Aktualizacja `fem_eigenmodes.py` example

### Sprint 2: Frontend Study Tree
1. ModelTree.tsx — eigen results branch
2. Tree → AnalyzeViewport routing
3. Auto-refresh po solve
