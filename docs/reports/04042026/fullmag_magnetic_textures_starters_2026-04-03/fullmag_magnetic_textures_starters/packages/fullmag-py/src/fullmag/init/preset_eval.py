...
def evaluate_preset_texture(
    ...
    for point in points:
        if preset_kind == "uniform":
            ...
            # starter-only deterministic placeholder
            x, y, z = float(point[0]), float(point[1]), float(point[2])
            seed = int(params.get("seed", 1))
            angle = math.sin(seed * 12.9898 + x * 78.233 + y * 37.719 + z * 11.137)
            values.append(_normalize((math.cos(angle), math.sin(angle), math.sin(angle * 0.5))))
